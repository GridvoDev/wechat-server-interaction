...
router.get('/suite-sys-event', (req, res)=> {
    ...
    if (isWeChatServerAuthURL) {
        ...
        logger.warn("weChat server had auth this url", traceContext);
    }
});
        ...
        wechatServerCallBackService.parseCallBackData(parseParameter, traceContext, (err, data)=> {
            ...
            if (data.InfoType == 'suite_ticket') {
                suiteSysEventHandleService.handleSuiteTicketArriveSysEvent(data, traceContext, (err, isSuccess)=> {
                    ...
                    if (!isSuccess) {
                        res.send('fail');
                        logger.warn("handle wechat server push suite_ticket sys event fail", traceContext);
                        return;
                    }
                    res.send('success');
                ...
                suiteSysEventHandleService.handleChangeAuthSysEvent(data, traceContext, (err, isSuccess)=> {
                    ...
                    if (!isSuccess) {
                        res.send('fail');
                        logger.warn("handle wechat server push change_auth sys event fail", traceContext);
                        return;
                    }
                    res.send('success');
                ...
                suiteSysEventHandleService.handleCancelAuthSysEvent(data, traceContext, (err, isSuccess)=> {
                    ...
                    if (!isSuccess) {
                        res.send('fail');
                        logger.warn("handle wechat server push cancel_auth sys event fail", traceContext);
                        return;
                    }
                    res.send('success');
                ...
                suiteSysEventHandleService.handleCreateAuthSysEvent(data, traceContext, (err, isSuccess)=> {
                    ...
                    if (!isSuccess) {
                        res.send('fail');
                        logger.warn("handle wechat server push create_auth sys event fail", traceContext);
                        return;
                    }
                    res.send('success');
                ...
                res.send('fail');
                logger.warn("no handle wechat server push sys event", traceContext);
            }
        });
    });
    req.once('error', err=> {
